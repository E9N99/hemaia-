...
async def mute(c, msg):
    if msg.reply_to_message.from_user.id in sudo_command:
        return await msg.edit("⎉ لا يمكنـك استخـدام الأمـر على مبرمـج السورس")
    chek = await is_Admin(msg.chat.id, msg.from_user.id)
    if chek == False:
        await message.reply("⎉ يجب ان تكون مشرف بالمجموعـة لاستخدام الاوامر")
        return False
    try:
        if msg.reply_to_message.from_user.id == sudo_id:
            return await msg.edit("⎉ لا يمكنك كتم نفسك")
        if msg.reply_to_message.from_user.id == 1260465030:
            return await msg.edit("⎉ لا يمكنك كتم مبرمـج السـورس")
        r.sadd(f"{sudo_id}mute{msg.chat.id}", msg.reply_to_message.from_user.id)
        txx = f"⎉ المستخـدم {get_name(msg.reply_to_message)} \n⎉ تم كتمه بنجاح"
        await msg.edit(txx)
    except:
        r.sadd(f"{sudo_id}mute{msg.chat.id}", msg.reply_to_message.sender_chat.id)
        await msg.edit("⎉ تم كتم القنـاة في المجموعـة")
...
async def un_mute(c, msg):
    ...
    try:
        ...
        if msg.reply_to_message.from_user.id == 1260465030:
            return await msg.edit("⎉ لا يمكنك الغاء كتم مبرمـج السـورس")
        ...
        await msg.edit("⎉ تم الغاء كتم القنـاة في المجموعـة")
...
async def bann(c, msg):
    ...
    if msg.reply_to_message.from_user.id == 1260465030:
        return await msg.edit("⎉ لا يمكنك حظر مبرمـج السـورس")
    try:
        await c.ban_chat_member(msg.chat.id, msg.reply_to_message.from_user.id)
        txx = f"⎉ المستخـدم {get_name(msg.reply_to_message)} \n⎉ تم حظره بنجاح"
        await msg.edit(txx)
    except Exception as e:
        await msg.edit(f"⎉ ليس لديك صلاحيات الحظر في المجموعـة")
...
async def un_ban(c, msg):
    ...
    if msg.reply_to_message.from_user.id == 1260465030:
        return await msg.edit("⎉ لا يمكنك الغاء حظر مبرمـج السـورس")
    try:
        await c.unban_chat_member(msg.chat.id, msg.reply_to_message.from_user.id)
        txx = f"⎉ المستخـدم {get_name(msg.reply_to_message)} \n⎉ تم الغاء حظره بنجاح"
        await msg.edit(txx)
    except:
        await msg.edit("⎉ المستخـدم ليس محظور \n⎉ أو ليـس لديـك صلاحيات الحظر في المجموعـة")
