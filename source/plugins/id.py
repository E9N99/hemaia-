...
async def get_info(c,msg):
  if msg.reply_to_message :
    if msg.reply_to_message.sender_chat :
      return await msg.edit("⎉ عـذرًا عـزيـزي هـذه قنـاة ..")
    ...
    if get_bio.bio :
      ...
      bio = "لا يوجد"
    txx = f"⎉ اسـمـه : {get_name(msg.reply_to_message)}\n⎉ معرفـه : {username} \n⎉ أيديـه : `{msg.reply_to_message.from_user.id}` \n⎉ البايـو : {bio}"
    ...
    if get_bio.username :
      ...
      username = "لا يوجد"
    txx = f"⎉ اسـمـه : {get_name(msg)}\n⎉ معرفـه : {username} \n⎉ أيديـه : `{msg.from_user.id}` \n⎉ البايـو : {bio}"
