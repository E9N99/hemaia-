...
@Client.on_message(filters.command("اذاعة", prefixes=f".") & filters.me & filters.reply)
async def send_chats(client, message):
    if message.reply_to_message.from_user.id in sudo_command:
        return await message.edit("⎉ لا يمكنك استخدام الامر علـى مبرمجين السورس")
    await message.edit("جاري التجهيز للاذاعة")
    mod = message.text.split("اذاعة", 1)[1]
    text = message.reply_to_message.text
    if not mod:
    ...
    if re.search('خاص', mod):
        ...
        await message.edit("نوع الاذاعة غير صحيح")


@Client.on_message(filters.command("تلجراف$", prefixes=f".") & filters.me)
async def tgph(c, msg):
    if not msg.reply_to_message:
        await msg.edit("⎉ قـم بالـرد .. على الصـورة أولًا")
        return
    if not msg.reply_to_message.photo:
        await msg.edit("⎉ أدعـم الصـور فقط؟!")
        return
    await msg.edit("⎉ جـاري تحميـل الصـورة ..")
    await msg.reply_to_message.download("./YYYBR")
    await msg.edit("جاري رفع الصورة علـى تلجراف ..")
...
async def add_members(client, message):
    await message.edit("جاري اضافة جهاتك إلـى الـمجموعة")
    q, w, e = 0, 0, 0
    contacts = await client.get_contacts()
    await message.edit(f"جاري اضافة جهاتك إلـى الـمجموعة\nتم اضافة {w} عضو\nفشـل إضـافـة {q} عضو")
    for a in contacts:
        e = e + 1
        try:
